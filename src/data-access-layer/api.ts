...
export const profileAPI = {
  ...
  updateProfilePhoto(formData: Blob) {
    return instanceForMedia
      .put<CommonResponseType<PhotoResponseType>>(`profile/photo`, formData)
  ...
  updateUserProfile(data: UpdateProfileRequestType) {
    return instance.put<CommonResponseType<{}>>('/profile', data).then((response) => response.data);
  },
};

export const usersAPI = {
  getUsers(portionSize: number, currentPage: number, term = '', friend: boolean | null = null) {
    return instance
      .get<UserResponseType>(
        `users?count=${portionSize}&page=${currentPage}&term=${term}` +
...
export const authAPI = {
  getAuth() {
    return instance
      .get<CommonResponseType<AuthResponseType>>(`auth/me`)
      .then((response) => response.data);
  },
  login(email: string, password: string, rememberMe: boolean, captcha = '') {
    return instance
      .post<CommonResponseType<{ userId: number } | {}>>(`auth/login`, {
        email,
        password,
        rememberMe,
        captcha,
