        ...
        if (response.resultCode === 0) {
          ...
          dispatch(
            stopSubmit('loginForm', {
              _error: response.messages[0] ? response.messages[0] : 'Some error',
            }),
          );
...
export const logOutThunk =
  (): ThunkType => (dispatch: ThunkDispatch<AppStateType, unknown, ActionsType | FormAction>) => {
...
export const getCaptchaTC =
  (): ThunkType => (dispatch: ThunkDispatch<AppStateType, unknown, ActionsType>) => {
