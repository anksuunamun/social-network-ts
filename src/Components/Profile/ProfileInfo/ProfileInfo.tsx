...
const ProfileInfo = function (props: ProfileInfoPropsType) {
  ...
  const onPhotoUpload = useCallback(
    (e: ChangeEvent<HTMLInputElement>) => {
      ...
      const newFileName = e.currentTarget.value.slice(12);
  ...
  return (
    <div className={styles.profileInfoWrapper}>
      <div className={styles.profilePhoto}>
        ...
        {props.isOwner ? (
          <label htmlFor="upload-profile-photo">
            <span>{fileName}</span> <PurpleButton text={'upload'} small />
      ...
      <div className={styles.textInfo}>
        ...
        />
        <ProfileContacts user={props.user} updateUserProfileTC={props.updateUserProfileTC} />
      </div>
    </div>
  );
};
