...
const LoginForm = React.memo(function (
  props: InjectedFormProps<FormDataType, AdditionalType> & AdditionalType,
) {
  return (
    <>
      <form onSubmit={props.handleSubmit}>
        {createField(
          ...
          'login',
          'text',
          [required, maxLength30, minLength4],
          'Login',
          'userLogin',
        )}
        {createField(
...
const Login = React.memo(function (
  props: LoginPropsType & MapDispatchToPropsType & MapStateToPropsType,
) {
  ...
  return (
    <div className={'contentWrapper'}>
      {props.isAuth ? (
        ...
        <LoginReduxForm onSubmit={onHandleSubmit} captchaURL={props.captchaURL} />
      )}
    </div>
  );
});
...
const mapDispatchToProps = (
  ...
  return {
    logInThunk: (email, password, rememberMe, captcha?: string) =>
